...
<body>
    ...
    $(document).ready(function () {
        $('#email').on('blur', function () {
            var email = $(this).val();
            $('#email-check').html('<i class="fa fa-spinner fa-spin"></i>'); // Show loading indicator
            ...
            if (email) {
                $.ajax({
                    ...
                    error: function () {
                        $('#email-check').html('<i class="fa fa-exclamation-triangle text-warning"></i>'); // Show error icon
                    }
                });
            } else {
                $('#email-check').html('');
